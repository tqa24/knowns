...
.PHONY: all build clean test test-e2e test-e2e-semantic test-e2e-ui lint dev dev-go dev-ui dev-all install cross-compile ui embed npm-build

all: ui build

# Development build (current platform)
build:
	go build -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY) ./cmd/knowns

# Development build with race detector
dev:
	go build -race -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY) ./cmd/knowns
...
install:
	go install -ldflags "$(LDFLAGS)" ./cmd/knowns
...
cross-compile: clean
	@for platform in $(PLATFORMS); do \
		...
		GOOS=$$os GOARCH=$$arch CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o $$output ./cmd/knowns; \
	done

# Build for npm distribution (maps to npm package names)
npm-build: clean
	@echo "Building for npm distribution..."
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-darwin-arm64/knowns ./cmd/knowns
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-darwin-x64/knowns ./cmd/knowns
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-linux-arm64/knowns ./cmd/knowns
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-linux-x64/knowns ./cmd/knowns
	GOOS=windows GOARCH=arm64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-win-arm64/knowns.exe ./cmd/knowns
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o npm/knowns-win-x64/knowns.exe ./cmd/knowns
